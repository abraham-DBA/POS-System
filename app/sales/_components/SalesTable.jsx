...
const SalesTable = () => {
    ...
    useEffect(() => {
        fetch('data/data.json')
            .then((res) => res.json())
            .then((data) => setSales(data.salesTransactions || []))
    }, [])

    const filteredSales = sales.filter((sale) =>
        sale.orderId.toString().includes(searchTerm.toLowerCase()) ||
                ...
                <Table>
                    <TableHeader className={`${isDark ? 'bg-[#2a2a2a] border-gray-700' : 'bg-gray-100 border-gray-300'} border-b`}>
                        <TableRow className='hover:bg-transparent'>
                            <TableHead className={`w-[80px] ${isDark ? 'text-white' : 'text-gray-900'} font-bold text-sm md:text-base`}>Order ID</TableHead>
                            <TableHead className={`${isDark ? 'text-white' : 'text-gray-900'} font-bold text-sm md:text-base`}>Customer</TableHead>
                            <TableHead className={`hidden sm:table-cell ${isDark ? 'text-white' : 'text-gray-900'} font-bold text-sm md:text-base`}>Product</TableHead>
                    ...
                    <TableBody>
                        {filteredSales.length > 0 ? (
                            filteredSales.map((sale) => (
                                <TableRow key={sale.id} className={`${isDark ? 'hover:bg-[#2a2a2a]' : 'hover:bg-gray-50'} transition duration-200`}>
                                    <TableCell className={`font-medium ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                                        #{sale.orderId}
                                    </TableCell>
                                    <TableCell className={`text-xs sm:text-sm ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{sale.customer}</TableCell>
                                    <TableCell className={`hidden sm:table-cell text-xs sm:text-sm ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{sale.product}</TableCell>
                                    <TableCell className={`hidden md:table-cell text-xs sm:text-sm font-semibold ${isDark ? 'text-green-400' : 'text-green-600'}`}>${sale.amount}</TableCell>
                                    <TableCell className={`hidden lg:table-cell text-xs sm:text-sm ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>{sale.date}</TableCell>
                                    <TableCell>
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                            sale.status === 'Completed' ? (isDark ? 'bg-green-900 text-green-200' : 'bg-green-100 text-green-800') :
                                            sale.status === 'Pending' ? (isDark ? 'bg-yellow-900 text-yellow-200' : 'bg-yellow-100 text-yellow-800') :
